window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function ServerForm({ initial, onDone, onCancel, scope, workspacePath, workspaces }) {
			...
			const submit = async () => {
				if (isWorkspace && !wsPath) { setError("请选择工作区"); return; }
				setBusy(true); setError("");
				try {
			...
			return createElement("div", { className: "mm_row mm_add" },
				createElement("div", { className: "mm_form" },
					createElement("label", { className: "wide" }, "作用域 Scope",
						createElement("select", { value: formScope, disabled: editing, onChange: (e) => setFormScope(e.target.value) },
							createElement("option", { value: "user" }, "user（全局，所有工作区可用）"),
							createElement("option", { value: "workspace" }, "workspace（仅指定工作区可用）"),
						),
					),
					isWorkspace ? createElement("label", { className: "wide" }, "工作区",
						createElement("select", { value: wsPath, disabled: editing, onChange: (e) => setWsPath(e.target.value) },
							createElement("option", { value: "" }, "选择工作区…"),
				...
				createElement("div", { className: "mm_actions" },
					createElement("button", { className: "mm_btn", onClick: submit, disabled: busy || !name || (isWorkspace && !wsPath) || (type === "stdio" ? !command : !url) }, busy ? "…" : "保存"),
					createElement("button", { className: "mm_btn", onClick: onCancel, disabled: busy }, "取消"),
				),
			);
		}

		function WorkspaceServerRow({ server, workspacePath, onChanged, onEdit }) {
			const [busy, setBusy] = react.useState(false);
			const [error, setError] = react.useState("");
			const [confirming, setConfirming] = react.useState(false);
			const startAuth = async () => {
			...
			return createElement("div", { className: "mm_row", key: server.name },
				createElement("div", { className: "mm_rowHead" },
					...
					createElement("span", { className: "mm_actions" },
						server.authMode === "oauth" && (server.status === "needs-auth" || server.status === "error" || server.status === "connected")
							...
							: null,
						createElement("button", { className: "mm_btn", onClick: onEdit, disabled: busy }, "编辑"),
						createElement("button", { className: "mm_btn danger", onClick: () => setConfirming(true), disabled: busy }, "删除"),
					),
				),
				createElement("div", { className: "mm_url" }, server.type === "stdio"
					? `stdio · ${server.command} ${(server.args || []).join(" ")}`
				...
				confirming ? createElement("div", { className: "mm_overlay", onClick: () => setConfirming(false) },
					createElement("div", { className: "mm_dialog", onClick: (e) => e.stopPropagation() },
						createElement("div", { className: "mm_dialogTitle" }, "删除工作区 MCP 服务器"),
						createElement("div", { className: "mm_dialogBody" }, `确定要从该工作区删除「${server.name}」吗？`),
						createElement("div", { className: "mm_dialogActions" },
							createElement("button", { className: "mm_btn", onClick: () => setConfirming(false), disabled: busy }, "取消"),
							createElement("button", { className: "mm_btn danger", onClick: remove, disabled: busy }, busy ? "…" : "删除"),
						),
		...
		function GlobalMaskRow({ server, excluded, onToggleExclude, onEdit, busy }) {
		...
		function McpSection() {
			...
			const toggleOnDemand = async () => {
				...
				try {
					...
					else setSettingsError(r.body.error || `切换失败 (HTTP ${r.status})`);
				} catch (e) { setSettingsError(String(e)); }
				setSettingsBusy(false);
			};

			const addBtn = createElement("button", { className: "mm_addBtn", type: "button", "aria-label": "添加 MCP 服务器", title: "添加 MCP 服务器", onClick: () => setView("add") },
			...
			if (view === "add") {
				return createElement("div", { className: "mm_section" },
					createElement("div", { className: "mm_catalogHeading" }, createElement("h3", null, "添加 MCP 服务器")),
					react.createElement(ServerForm, { scope: selected ? "workspace" : "user", workspacePath: selected || "", workspaces: workspaces.map((w) => w.path), onDone: () => { setView("list"); refresh(); }, onCancel: () => setView("list") }),
				);
			}
			const globalEditing = view === "edit-global" ? (servers.find((s) => s.id === editingId) ?? null) : null;
			if (view === "edit-global" && globalEditing) {
				return createElement("div", { className: "mm_section" },
					createElement("div", { className: "mm_catalogHeading" }, createElement("h3", null, "编辑 MCP 服务器（全局）")),
					react.createElement(ServerForm, { initial: globalEditing, scope: "user", onDone: () => { setView("list"); refresh(); }, onCancel: () => setView("list") }),
				);
			}
			...
			if (view === "edit-ws" && selected && wsEditing) {
				return createElement("div", { className: "mm_section" },
					createElement("div", { className: "mm_catalogHeading" }, createElement("h3", null, "编辑 MCP 服务器（工作区）")),
					createElement("div", { className: "mm_wsPathHint" }, selected),
					react.createElement(ServerForm, { initial: wsEditing, scope: "workspace", workspacePath: selected, onDone: () => { setView("list"); refresh(); }, onCancel: () => setView("list") }),
				);
			}
			...
			return createElement("div", { className: "mm_section" },
				...
				createElement("div", { className: "mm_feature" },
					createElement("span", { className: "mm_featureText" },
						createElement("span", { className: "mm_featureTitle" }, "按需 MCP 工具调用"),
						createElement("span", { className: "mm_featureMeta" }, "Native 模式下，MCP 仅暴露 search、describe、execute 三个 broker 工具"),
					),
					createElement("span", { className: "mm_switchRow" },
						createElement("button", { className: "mm_switch", type: "button", role: "switch", "data-on": settings.onDemandToolInjection ? "true" : undefined, "aria-checked": settings.onDemandToolInjection, "aria-label": "按需 MCP 工具调用", onClick: toggleOnDemand, disabled: settingsBusy },
							createElement("span", { className: "mm_switchThumb" }),
						),
						createElement("span", { className: "mm_switchText" }, settings.onDemandToolInjection ? "已开启" : "已关闭"),
					),
				),
				...
				createElement("div", { className: "mm_wsBar" },
					createElement("select", { className: "mm_wsSelect", value: selected, onChange: (e) => { setSelected(e.target.value); setExpandedId(null); setQuery(""); } },
						createElement("option", { value: "" }, "全局（所有工作区）"),
				...
				selected
					? [
						createElement("div", { className: "mm_groupTitle" }, "工作区服务器（仅此工作区可见）"),
						wsServers.length > 0
							? createElement("div", { className: "mm_wsList" },
								wsServers.map((s) => react.createElement(WorkspaceServerRow, { server: s, workspacePath: selected, onChanged: refresh, onEdit: () => { setEditingName(s.name); setView("edit-ws"); }, key: s.name })),
							)
							: createElement("div", { className: "mm_meta" }, "该工作区还没有专属 MCP 服务器，点右上角 ＋ 添加（写入 .dsh/dshmm/mcp.json）。"),
						createElement("div", { className: "mm_groupTitle" }, "全局服务器（在此工作区可用）"),
						servers.length > 0
							? createElement("div", { className: "mm_wsList" },
								servers.map((g) => react.createElement(GlobalMaskRow, { server: g, excluded: excludeSet.has(g.name), onToggleExclude: toggleExclude, onEdit: () => { setEditingId(g.id); setView("edit-global"); }, busy: excludeBusy, key: g.id })),
							)
							: createElement("div", { className: "mm_meta" }, "还没有全局服务器。切换到「全局」视图添加。"),
					]
					: [
						...
						createElement("div", { className: "mm_cards" },
							filteredServers.map((s) => react.createElement(ServerRow, { server: s, onChanged: refresh, onEdit: () => { setEditingId(s.id); setView("edit-global"); }, open: expandedId === s.id, onToggle: () => setExpandedId(expandedId === s.id ? null : s.id), key: s.id })),
						),
					],
			);
		}
