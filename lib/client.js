window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function McpSection() {
			const [language, setLanguage] = react.useState(systemLanguage);
			...
			react.useEffect(() => {
				...
				api("/settings").then((r) => {
					if (!active) return;
					if (r.ok) {
						if (r.body.language === "zh" || r.body.language === "en") setLanguage(r.body.language);
					} else setError(t("languageLoadFailed", { status: r.status }));
