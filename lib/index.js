...
export function apply(ctx) {
  ...
  async function discoverProtectedResource(server) {
    ...
    try {
      ...
      const m = /resource_metadata="([^"]+)"/i.exec(resp.headers.get('www-authenticate') ?? '');
      if (m) urls.push(m[1]);
    ...
    for (const u of urls) {
      ...
      if (Array.isArray(md?.authorization_servers) && md.authorization_servers.some((s) => typeof s === 'string')) return md;
    }
    return null;
  }
  ...
  async function discoverOauthMetadata(server) {
    ...
    // 1. Explicit `oauth.issuer` override, or the MCP origin acting as its own AS.
    let md = await fetchAuthServerMetadata(issuer);
    if (md) return md;
    // 2. Follow protected-resource metadata to the real authorization server.
    if (!server.oauth?.issuer) {
      const prm = await discoverProtectedResource(server);
      for (const as of prm?.authorization_servers ?? []) {
        if (typeof as !== 'string') continue;
        md = await fetchAuthServerMetadata(as);
        if (md) {
          if (Array.isArray(prm.scopes_supported) && prm.scopes_supported.length) md.dsh_resource_scopes = prm.scopes_supported.filter((s) => typeof s === 'string');
          return md;
        }
      }
    }
