...
export function apply(ctx) {
  ...
  // Follow exactly one same-origin 307/308 — typically a trailing-slash
  // normalization such as `/mcp/` → `/mcp` (Next.js/Vercel answer 308).
  // Cross-origin redirects are never followed: doing so would hand the
  // Authorization header to another host.
  function sameOriginRedirect(url, resp) {
    if (resp.status !== 307 && resp.status !== 308) return null;
  ...
  async function httpPostJson(url, headers, body, timeoutMs = 60000) {
    ...
    try {
      ...
      const next = sameOriginRedirect(url, resp);
      if (next) {
        await resp.body?.cancel().catch(() => {});
        resp = await fetch(next, init);
      }
  ...
  // RFC 8414 / OIDC discovery for an issuer that may carry a path component
  // (e.g. https://host/auth/v1). Try the path-insertion form first, then the
  // path-suffix form, then OpenID configuration.
  async function fetchAuthServerMetadata(issuer) {
    let u;
    try { u = new URL(issuer); } catch { return null; }
    ...
    for (const c of candidates) {
      const md = await fetchJsonOk(c);
      if (md && md.authorization_endpoint && md.token_endpoint) return md;
    }
    return null;
  }

  // RFC 9728: the MCP endpoint's 401 carries
  //   WWW-Authenticate: Bearer resource_metadata="<url>"
  // pointing at the protected-resource document, which names the
  // authorization server(s). Fall back to the well-known PRM locations.
  async function discoverProtectedResource(server) {
    let base;
    try { base = new URL(server.url); } catch { return null; }
    const urls = [];
    let challengeScope = null;
    try {
      ...
      const m = /resource_metadata="([^"]+)"/i.exec(challenge);
      if (m) urls.push(m[1]);
      const sc = /\bscope="([^"]*)"/i.exec(challenge);
      if (sc && sc[1].trim()) challengeScope = sc[1].trim().split(/\s+/);
      try { await resp.body?.cancel(); } catch {}
    } catch {}
    const path = base.pathname.replace(/\/$/, '');
    if (path) urls.push(`${base.origin}/.well-known/oauth-protected-resource${path}`);
    urls.push(`${base.origin}/.well-known/oauth-protected-resource`);
    for (const u of urls) {
      const md = await fetchJsonOk(u);
      if (Array.isArray(md?.authorization_servers) && md.authorization_servers.some((s) => typeof s === 'string')) {
        return { metadata: md, scope: challengeScope };
      }
    }
    return challengeScope ? { metadata: null, scope: challengeScope } : null;
  }

  async function discoverOauthMetadata(server) {
    const issuer = issuerOf(server);
    // The protected-resource document (RFC 9728) is the authoritative source
    // for (a) where the authorization server lives when it is not the MCP
    // origin and (b) which scopes to request. Some providers (Perplexity)
    // publish AS metadata on the MCP origin *and* still require a scope, so
    // consult it even when the origin resolves on its own. An explicit
    // `oauth.issuer` override skips this and uses the issuer verbatim.
    const resource = server.oauth?.issuer ? null : await discoverProtectedResource(server);
    const prm = resource?.metadata ?? null;
    // Scope priority: WWW-Authenticate `scope` param, then PRM scopes_supported.
    const scopes = resource?.scope
      ?? (Array.isArray(prm?.scopes_supported) ? prm.scopes_supported.filter((s) => typeof s === 'string' && s) : null);
    const withScopes = (md) => {
      if (scopes && scopes.length) md.dsh_resource_scopes = scopes;
      return md;
    };
    // 1. Explicit issuer override, or the MCP origin acting as its own AS.
    let md = await fetchAuthServerMetadata(issuer);
    if (md) return withScopes(md);
    // 2. Authorization server(s) named by the protected resource.
    for (const as of prm?.authorization_servers ?? []) {
      if (typeof as !== 'string') continue;
      md = await fetchAuthServerMetadata(as);
      if (md) return withScopes(md);
    }
    // 3. Legacy guess: conventional paths on the issuer.
    return withScopes({
  ...
  async function startAuth(server, origin) {
    ...
    if (Array.isArray(md.dsh_resource_scopes) && md.dsh_resource_scopes.length) url.searchParams.set('scope', md.dsh_resource_scopes.join(' '));
    ...
    void (async () => {
      while (!controller.signal.aborted && !handle.closed) {
        try {
          ...
          const redirected = sameOriginRedirect(server.url, response);
          if (redirected) {
          ...
          if (response.status === 401 && server.authMode === 'oauth' && (await refreshTokens(server))) {
            await response.body?.cancel().catch(() => {});
            response = await request(redirected ?? server.url);
          }
