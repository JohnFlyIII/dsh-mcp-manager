...
export function apply(ctx) {
  ...
  function registerToolsGlobal(server, conn, tools, callFn) {
  ...
  async function openHttp(server) {
    const handle = { kind: 'http', sessionId: null, transport: null, tools: [], call: () => Promise.reject(new Error('not connected')), close: () => {} };
    ...
    const listed = await mcpRpc(server, 'tools/list', {}, handle);
  ...
  async function openStdio(server) {
    ...
    const handle = { kind: 'stdio', sessionId: null, transport, tools: [], call: () => Promise.reject(new Error('not connected')), close: () => {} };
    try {
  ...
  async function connect(server) {
    ...
    try {
      ...
      registerToolsGlobal(server, conn, handle.tools, handle.call);
    } catch (error) {
      for (const dispose of conn.tools.values()) { try { dispose(); } catch {} }
      conn.tools = new Map();
      conn.toolCount = 0;
  ...
  function registerWorkspaceTools(toolsRegistry, server, wsConn) {
    const disposers = [];
    for (const tool of wsConn.tools) {
      disposers.push(toolsRegistry.register(makeToolDefinition(server, tool, wsConn.call)));
    }
    return disposers;
  }

  function disposeAgentScope(st) {
    for (const disposers of st.disposers.values()) for (const d of disposers) { try { d(); } catch {} }
    st.disposers.clear();
  ...
  function rebuildAgentWorkspace(agent, wsPath) {
    ...
    if (st) disposeAgentScope(st);
    if (!ws) { if (st) agentScopeState.delete(agent); return; }
    st = { wsPath, disposers: new Map(), restrictDisposer: undefined };
    agentScopeState.set(agent, st);
    for (const [name, wc] of ws.servers) {
      if (wc.status !== 'connected') continue;
      try {
        st.disposers.set(name, registerWorkspaceTools(agent.ctx.tools, wc.server, wc));
      } catch (error) {
    ...
    return async (agentCtx) => {
      ...
      if (typeof cwd === 'string' && cwd.length > 0 && isAbsolute(cwd)) {
        try {
          ...
          ws.agents.add(agent);
          agentScopeState.set(agent, { wsPath, disposers: new Map(), restrictDisposer: undefined });
    ...
    async handler(req, res) {
      ...
      try {
        ...
        if (req.method === 'GET' && rest === '/ping') {
          return json(res, 200, { ok: true, version: 3, stdio: true, workspace: true });
        }
