 ...
 *   GET  /mcp-manager/api/settings        profile-level feature and language settings
 *   POST /mcp-manager/api/settings/language   persist UI language (zh/en)
    ...
    async handler(req, res) {
      ...
      try {
        ...
        if (req.method === 'GET' && rest === '/settings') {
          return json(res, 200, { onDemandToolInjection: state.onDemandToolInjection, language: state.language === 'zh' || state.language === 'en' ? state.language : null });
        }
        if (req.method === 'POST' && rest === '/settings/language') {
          const body = await readBody(req);
          if (body?.language !== 'zh' && body?.language !== 'en') {
            return json(res, 400, { error: 'language must be zh or en' });
          }
          // Persist before changing live state so failed writes do not claim success.
          saveState({ ...state, language: body.language });
          state.language = body.language;
          return json(res, 200, { language: state.language });
        }
